...
#define SHFL_SYNC(VAL, SRC_LANE, WIDTH) \
    __shfl_sync(kWarpAllMask, VAL, SRC_LANE, WIDTH)
#else
...
#if CUDA_VERSION >= 9000
    return __shfl_sync(kWarpAllMask, val, srcLane, width);
#else
...
#if CUDA_VERSION >= 9000
    return __shfl_up_sync(kWarpAllMask, val, delta, width);
#else
...
#if CUDA_VERSION >= 9000
    return __shfl_down_sync(kWarpAllMask, val, delta, width);
#else
...
#if CUDA_VERSION >= 9000
    return __shfl_xor_sync(kWarpAllMask, val, laneMask, width);
#else
